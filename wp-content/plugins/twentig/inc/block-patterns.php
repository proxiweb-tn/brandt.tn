...
function twentig_register_block_pattern( $pattern_name, $pattern_properties ) {
	...
	if ( wp_is_block_theme() && ! current_theme_supports( 'twentig-theme' ) ) {
		$pattern_properties['content'] = twentig_replace_pattern_preset_to_values( $pattern_properties['content'] );
	}
	if ( ! current_theme_supports( 'twentig-theme' ) && ! in_array( $theme, array( 'twentytwentythree', 'twentytwentytwo', 'twentytwentyone', 'twentytwenty' ), true ) ) {
		$pattern_properties['content'] = twentig_replace_pattern_opiniated_style( $pattern_properties['content'] );
	} elseif ( 'twentytwentytwo' === $theme ) {
		$pattern_properties['content'] = twentig_replace_pattern_twentytwo( $pattern_properties['content'] );
	} elseif ( 'twentytwenty' === $theme ) {
		$pattern_properties['content'] = twentig_replace_pattern_twentytwenty( $pattern_properties['content'] );
	}	
...
function twentig_replace_pattern_preset_to_values( $content ) {
	...
	$strings_replace = array(
		...
		array(
			'old' => '<!-- wp:paragraph {"style":{"typography":{"lineHeight":"1.1"}},"fontSize":"5-x-large"} --><p class="has-5-x-large-font-size" style="line-height:1.1">',
			'new' => '<!-- wp:paragraph {"style":{"typography":{"lineHeight":"1.1","fontSize":"4rem"}}} --><p style="font-size:4rem;line-height:1.1">',
		),
		array(
		...
		array(
			'old' => 'has-secondary-color has-text-color ',
			'new' => '',
		),
		array(
			'old' => ' class="has-secondary-color has-text-color"',
			'new' => '',
		),
	);

	$old_strings = array_column( $strings_replace, 'old' );
	$new_strings = array_column( $strings_replace, 'new' );
	$content     = str_replace( $old_strings, $new_strings, $content );
...
/**
 * Replaces pattern opiniated styles for non Twentig theme nor default themes.
 */
function twentig_replace_pattern_opiniated_style( $content ) {
...
/**
 * Replace pattern colors for Twenty Twenty-Two.
 */
function twentig_replace_pattern_twentytwo( $content ) {
...
/**
 * Replaces pattern values for Twenty Twenty.
 */
function twentig_replace_pattern_twentytwenty( $content ) {
